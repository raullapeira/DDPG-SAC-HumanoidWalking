"""
Records a long GIF of the 500k checkpoint policy.
"""
import sys, os
_HERE = os.path.dirname(os.path.abspath(__file__))
sys.path.insert(0, os.path.join(_HERE, ".."))

import numpy as np
import torch
import torch.nn as nn
import imageio
from PIL import Image, ImageDraw, ImageFont
import gymnasium as gym
import mujoco
from envs.alpha_env import AlphaEnv

CKPT      = os.path.join(_HERE, "..", "checkpoints", "sac_alpha", "sac2_checkpoint_900000.pt")
OUT_GIF   = os.path.join(_HERE, "..", "media", "alpha_900k_walking.gif")
SECONDS   = 20
FPS       = 30
WIDTH     = 640
HEIGHT    = 480
DEVICE    = torch.device("cuda" if torch.cuda.is_available() else "cpu")


class Actor(nn.Module):
    def __init__(self, s, a, m):
        super().__init__()
        self.net = nn.Sequential(nn.Linear(s,256),nn.ReLU(),nn.Linear(256,256),nn.ReLU())
        self.mu  = nn.Linear(256, a)
        self.log_std = nn.Linear(256, a)
        self.max_action = m
    def act(self, state):
        with torch.no_grad():
            return torch.tanh(self.mu(self.net(state))) * self.max_action


def add_label(frame, text):
    img  = Image.fromarray(frame)
    draw = ImageDraw.Draw(img)
    draw.rectangle([0, 0, WIDTH, 34], fill=(0, 0, 0))
    try:
        font = ImageFont.truetype("arial.ttf", 22)
    except Exception:
        font = ImageFont.load_default()
    draw.text((10, 6), text, fill=(255, 255, 255), font=font)
    return np.array(img)


env = gym.wrappers.TimeLimit(AlphaEnv(), max_episode_steps=10_000)
s = env.observation_space.shape[0]
a = env.action_space.shape[0]
m = float(env.action_space.high[0])

actor = Actor(s, a, m).to(DEVICE)
ckpt  = torch.load(CKPT, map_location=DEVICE, weights_only=False)
actor.load_state_dict(ckpt["actor"])
actor.eval()

renderer = mujoco.Renderer(env.unwrapped.model, height=HEIGHT, width=WIDTH)
cam = mujoco.MjvCamera()
cam.type      = mujoco.mjtCamera.mjCAMERA_FREE
cam.lookat[:] = [0.0, 0.0, 0.20]
cam.distance  = 0.9
cam.azimuth   = 150
cam.elevation = -18

# env steps between captured frames  (control dt = 0.025s → 40 env-steps/sec)
env_steps_per_frame = max(1, round(40 / FPS))
n_frames = SECONDS * FPS

obs, _ = env.reset(seed=0)
frames = []
total_reward = 0
ep = 1

print(f"Recording {SECONDS}s / {n_frames} frames from {CKPT} ...")
for fi in range(n_frames):
    for _ in range(env_steps_per_frame):
        st = torch.FloatTensor(obs).unsqueeze(0).to(DEVICE)
        action = actor.act(st).cpu().numpy()[0]
        obs, r, term, trunc, info = env.step(action)
        total_reward += r
        if term or trunc:
            obs, _ = env.reset(seed=ep)
            ep += 1

    renderer.update_scene(env.unwrapped.data, camera=cam)
    frame = renderer.render().copy()
    step_label = os.path.basename(CKPT).replace("sac2_checkpoint_", "Step ").replace(".pt", "")
    label = f"{step_label}  |  ep {ep}  |  x_vel {info['x_velocity']:+.2f} m/s"
    frames.append(add_label(frame, label))

env.close()
print(f"Saving {len(frames)} frames → {OUT_GIF}")
imageio.mimsave(OUT_GIF, frames, fps=FPS, loop=0)
print("Done.")
